...
echo "  4) native     (host CLI, no container sandbox)"
echo ""
...
if [[ "$AR_CONTAINER_RUNTIME" == "native" ]]; then
    echo "  Native mode has no Agentic Researcher filesystem sandbox."
...
# ── 6. GPU backend ───────────────────────────────────────────────────
echo "─── GPU Job Backend ───"
echo "  1) auto         (native: use cluster-run if available; otherwise none)"
echo "  2) none         (local/manual GPU use only)"
echo "  3) cluster-run  (native cluster-run command)"
echo "  4) remote-run   (Apptainer + Slurm multi-node dispatch)"
echo ""
read -rp "Select [1]: " gpu_backend_choice
case "${gpu_backend_choice:-1}" in
    1) AR_GPU_BACKEND=auto ;;
    2) AR_GPU_BACKEND=none ;;
    3) AR_GPU_BACKEND=cluster-run ;;
    4) AR_GPU_BACKEND=remote-run ;;
    *) echo "Invalid choice, defaulting to auto"; AR_GPU_BACKEND=auto ;;
esac
echo "  → $AR_GPU_BACKEND"
echo ""
