...
def render_instruction(ctx: Any) -> str:
    ...
    return "\n".join(
        [
            "## Experiment Log",
            "",
            f"When this workflow records experiments, the active work-branch experiment log lives on the project work state branch `agentic/work-state/{work_branch}` at `experiment-log/`. The log is capability-owned and may not exist until the first experiment is recorded. Use the `experiment-logger` subagent for new experiment results and the `experiment-corrector` subagent for corrections. These are required subagent handoffs in generated instructions: try to spawn the named subagent, retry once if spawning fails, and alert the user if it still cannot be spawned rather than silently calling `experiment-log` directly. Within this work-branch log, experiment IDs are local, such as `E0001_block-sparse-baseline`; from other work-branch logs, refer to them as `{work_branch}::E0001_block-sparse-baseline`.",
            "",
