...
Org-provided agents live inside `capabilities/<capability>/agents/`. A capability may also ship structured Python tools, workflows, and lifecycle hooks under `package/`, skills under `skills/`, commands under `bin/`, private helpers under `lib/`, and launcher hooks under `launcher/`. The launcher exposes enabled package roots through `$AR_TOOL_PATH`; `[tools]` entries in `capability.toml` give PythonTools stable public names for request-scoped imperative-workflow grants. They are not exposed to agents globally through MCP. The JSON/YAML `agentic-tool module:Class` adapter remains available for explicit shell callers. Package-native `[hooks]` entries replace shell wrappers for lifecycle work while legacy `hooks/` executables remain a migration fallback. `capability.toml` also declares transitive dependencies. Capabilities are selected explicitly or by selecting a main agent they provide. Project providers use the same structure under `.agentic-team/capabilities/` and take precedence over org and built-in providers.
...
- `branch` provides native Python snapshot, commit, status, cleanup, and temporary-worktree tools shared by workflows and available to explicitly granted agent requests, plus thin command wrappers for compatibility and durable workers.
